...
class DebugTool {
    fun buildVersion(): String {
        // 构建版本 ： v_1_1.0.1
        return "构建版本:v_" + cn.jack.debugtoolu.BuildConfig.VERSION_CODE + "_" + cn.jack.debugtoolu.BuildConfig.VERSION_NAME
    }

    fun buildEnvironment(): String {
        return "构建环境: " + if (cn.jack.debugtoolu.BuildConfig.DEBUG) "测试环境" else "正式环境"
    }

    fun buildTime(): String {
        //new date() 当前你在运行时拿到的时间，这个包，被打出来的时间
        return "构建时间：" + cn.jack.debugtoolu.BuildConfig.BUILD_APK_TIME
    }
